...
// Embed define
      const sucess = new MessageEmbed()
          .setColor('#0099ff')
          .setTitle('Sucess!')
          .setDescription('Vous avez accepté les règles avec succès !');
    if (!interaction.isButton()) return;
        if (interaction.isButton("rulesa")) {
        console.log("L'utilisateur a accepté les règles !")
        interaction.deferUpdate();
	const user = client.users.cache.get(interaction.user.id);
	user.send("Vous avez accepté avec succès les règles").catch(console.error);
    	    const linkID = pool.createLink(user.id);
    const embed2 = new MessageEmbed()
        .setTitle('reCAPTCHA Verification')
        .setDescription(`Pour accéder à ce serveur, vous devez résoudre un captcha. Le lien expirera dans 15 minutes.\n${config.https ? 'https://' : 'http://'}${config.domain}/verify/${linkID}`)
        .setColor('BLUE');
    user.send({ embeds: [embed2] }).catch(() => {
        logger.error(`Échec de l'envoi du captcha à l'utilisateur ! (Peut-être qu'ils ont désactivé les DM ?)`);
})
}
});
...
client.on("messageCreate", async (message) => {
  ...
  if (!message.content.startsWith(prefix) || message.author.bot) return;
    ...
    if (message.member.roles.cache.has(config.discord['verified-role-id'])) {
        message.channel.send('Oups, vous êtes déjà vérifié!')
        return;
    }
